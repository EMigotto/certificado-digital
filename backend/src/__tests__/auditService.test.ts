...
describe('mapToApiAuditEntry', () => {
  it('should map Prisma AuditLog to API AuditLogEntry with ISO timestamp', () => {
  ...
  it('should handle null certId', () => {
    const log = makeAuditLog({ certId: null });
    const result = mapToApiAuditEntry(log);
    expect(result.certId).toBeNull();
  });

  it('should handle FAILURE result', () => {
    const log = makeAuditLog({ result: 'FAILURE', detail: 'Import failed: invalid cert' });
    const result = mapToApiAuditEntry(log);
...
describe('AuditService.log', () => {
  ...
  it('should include both batchId and errorReason when present', async () => {
    const created = makeAuditLog({ detail: 'Row failed | batch: b-1 | error: parse error' });
    mocks.create.mockResolvedValue(created);
  ...
  it('should produce empty detail when no detail/batch/error provided', async () => {
    const created = makeAuditLog({ detail: '' });
    mocks.create.mockResolvedValue(created);
...
describe('AuditService.getEntries', () => {
  ...
  it('should map multiple entries correctly', async () => {
    const entries = [
      makeAuditLog({ id: 'a-1', action: 'CREATE' }),
      makeAuditLog({ id: 'a-2', action: 'DELETE' }),
      makeAuditLog({ id: 'a-3', action: 'REVOKE', result: 'FAILURE' }),
    ];
...
describe('AuditService.getByBatchId', () => {
  ...
  it('should return all entries for a given batch ID', async () => {
    const entries = [
      makeAuditLog({ id: 'a-1', detail: 'CSV bulk import (batch: batch-abc)' }),
      makeAuditLog({ id: 'a-2', detail: 'CSV bulk import (batch: batch-abc)' }),
    ];
  ...
  it('should map entries to API format with ISO timestamps', async () => {
    const entries = [makeAuditLog({ timestamp: new Date('2025-06-15T14:00:00.000Z') })];
    mocks.findByBatchId.mockResolvedValue(entries);
